...
var CLIENT_ID =
  "462660603403-khj5nlahao22ed684s838h5a2k734uhf.apps.googleusercontent.com";
      ...
      if (assetCount > 0) {
        for (let i = 0; i < assetCount; i++) {
          ...
          if (name.length < 7) {
            ...
            name =
              name.substring(0, 5) +
              ("0" + name.substring(5, name.length)).slice(-2);
...
function renderDesksWithEvents(targetDate) {
  ...
  assets.then((assets) => {
    ...
    if (assets.length > 0) {
      ...
      let message =
        new Date(week[0]).toLocaleDateString() +
        " - " +
        new Date(week[1]).toLocaleDateString();
      ...
      Promise.all(deskEvents).then((events) => {
        ...
        for (let i = 1; i < 6; i++) {
          ...
          let textContent = document.createTextNode(
            weekDays[i].toLocaleDateString("en-GB")
          );
  ...
  assets.catch((err) => {
    console.error(
      "listDesks",
      "There was an error processing the desks. " + err
    );
  });
}
